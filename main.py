...
CEREBRAS_RATE_LIMIT_COOLDOWN_SECONDS = 120
CEREBRAS_REQUEST_TIMEOUT = 25.0
REQUIRE_DISTINCT_APPLY_URL = True
SOURCE_FAILURE_COOLDOWN_MINUTES = 90
DIRECT_SOURCE_TIMEOUT_SECONDS = 12
FRAGILE_DIRECT_SOURCE_TIMEOUTS = {
    "Smart Job": 8,
    "Dohaj": 8,
}
TARGETED_LANE_LIMIT = 4
MAX_APPLY_EXA_FALLBACKS = 12
...
# Listing portals are discovery inputs, not critical article delivery paths.
# Use a no-retry session there so a dead/403/404 portal cannot consume minutes
# before the other sources are processed.
direct_session = requests.Session()
direct_session.headers.update(HEADERS)
direct_adapter = HTTPAdapter(max_retries=Retry(total=0, connect=0, read=0, redirect=0))
...
def _safe_url_parts(url):
    """Parse a URL and return a requests/trafilatura-safe URL.

    Python HTTP stacks eventually encode request targets as latin-1 unless the
    path has already been percent-encoded. Career/news pages frequently contain
    Bangla slugs, so keep the semantic URL but encode non-ASCII path characters.
    """
    raw = safe_text(url)
    if not raw:
        return None
...
def canonical_url(url):
    ...
    if not parts:
        return ""
    scheme, netloc, path, query = parts
    host = netloc.lower().removeprefix("www.").removeprefix("amp.")
...
exa = None
cerebras = None
cerebras_rate_limited_until = 0.0
apply_exa_fallback_used = 0
...
def exa_search_with_contents(query, **kwargs):
    """Use the current Exa search API; retain one compatibility fallback.

    Newer exa-py releases expose search(..., contents=...). Older pinned
    environments may still expose search_and_contents(), so fall back only when
    the primary method rejects the call shape.
    """
    client = get_exa()
    try:
...
def targeted_portal_lane_gap_fill(region="Career", max_total=36):
    ...
    for lane,query in TARGETED_JOB_LANES:
        ...
        try:
            results=exa_search_with_contents(
...
def enrich_thin_excerpt(item):
    ...
    try:
        downloaded = trafilatura.fetch_url(fetchable_url(item["url"]))
        if not downloaded:
            return None
...
def _rank_batch(batch, region, batch_no):
    ...
    try:
        ...
        if status == 429 or "429" in safe_text(exc) or "rate limit" in safe_text(exc).lower():
            global cerebras_rate_limited_until
            cerebras_rate_limited_until = max(cerebras_rate_limited_until, time.monotonic() + CEREBRAS_RATE_LIMIT_COOLDOWN_SECONDS)
            logger.warning("Cerebras rate limited on ranking batch %d; deterministic ranking will be used for remaining batches for %ds.", batch_no, CEREBRAS_RATE_LIMIT_COOLDOWN_SECONDS)
        else:
...
def rank_candidates(candidates, region):
    ...
    for offset in range(0,len(regional),CEREBRAS_RANK_BATCH_SIZE):
        ...
        if time.monotonic() < cerebras_rate_limited_until:
            logger.info("Skipping Cerebras ranking batch %d while rate-limit cooldown is active.", offset//CEREBRAS_RANK_BATCH_SIZE+1)
            rows=[]
        else:
...
def _apply_url_candidates_from_blob(page_url, blob):
    """Extract full and relative application routes embedded in attributes/JS."""
    raw = html.unescape(safe_text(blob))
    if not raw:
        return []
    found=[]
    patterns=[
        ...
        r"(?:window\.)?(?:location(?:\.href)?|url|href|target)\s*[:=]\s*['\"]([^'\"]+)['\"]",
        r"['\"]((?:/|\.\.?/)[^'\"]*(?:apply|application|jobapply|submit|candidate)[^'\"]*)['\"]",
    ]
    for pattern in patterns:
        for match in re.finditer(pattern,raw,re.I):
            value=match.group(0) if pattern.startswith("https?") else match.group(1)
            value=safe_text(value).strip("'\" ,;)")
            if not value or value.lower().startswith(("javascript:","mailto:","tel:","#")):
                continue
            absolute=urljoin(page_url,value)
            distinct=distinct_http_url(absolute,page_url)
            if distinct and is_actionable_apply_url(distinct):
                found.append(distinct)
    return list(dict.fromkeys(found))


def extract_labelled_apply_url(text, page_url):
    """Read explicit 'Apply URL'/'Application URL' fields from extracted text."""
    raw=html.unescape(safe_text(text))
    if not raw:
        return ""
    label_re=r"(?:apply\s*(?:url|link)|application\s*(?:url|link)|apply\s*online)"
    patterns=[
        label_re+r"\s*[:\-]\s*(https?://[^\s<>'\")]+)",
        label_re+r"\s*[:\-]\s*((?:/|\.\.?/)[^\s<>'\")]+)",
    ]
    for pattern in patterns:
        m=re.search(pattern,raw,re.I)
        if m:
            candidate=distinct_http_url(urljoin(page_url,m.group(1)),page_url)
...
def extract_apply_url_from_attributes(page_url, soup):
    candidates=[]
    attr_names=("data-apply-url","data-apply","data-application-url","data-application","data-href","data-url","data-link","data-target","href","action")
    for tag in soup.find_all(["a","button","input","form"]):
        ...
        combined=label+" "+blob+" "+onclick
        score=0
        if re.search(r"(apply now|apply here|apply online|apply|application|submit|আবেদন|আবেদন করুন|এখনই আবেদন)",label,re.I): score+=14
        if re.search(r"(apply|application|jobapply|submit|recruit|candidate|career|vacancy)",combined,re.I): score+=8
        for a in attr_names:
            raw=safe_text(tag.get(a))
            if not raw: continue
            for u in _apply_url_candidates_from_blob(page_url,raw):
                candidates.append((score+10,len(u),u))
            if a in {"href","data-apply-url","data-apply","data-application-url","data-application","data-href","data-url","data-link","data-target","action"}:
                u=distinct_http_url(urljoin(page_url,html.unescape(raw)),page_url)
                if u and is_actionable_apply_url(u):
                    candidates.append((score,len(u),u))
        for u in _apply_url_candidates_from_blob(page_url,onclick):
            candidates.append((score+10,len(u),u))
    candidates.sort(key=lambda x:(-x[0],x[1]))
    return candidates[0][2] if candidates else ""

def exa_apply_url_fallback(page_url, article_text=""):
    """Bounded last-resort recovery for dynamic job portals whose Apply control
    is not present as a normal href/button in the downloaded HTML.
    """
    global apply_exa_fallback_used
    if apply_exa_fallback_used >= MAX_APPLY_EXA_FALLBACKS:
        return ""
    apply_exa_fallback_used += 1
    try:
        result_set = get_exa().get_contents([fetchable_url(page_url)], text={"max_characters": 7000})
        for result in getattr(result_set, "results", []) or []:
            text = safe_text(getattr(result, "text", ""))
            if not text:
                continue
            candidate = extract_labelled_apply_url(text, page_url)
            if candidate:
                return candidate
            for candidate_url in extract_urls_near_apply(text):
                candidate = distinct_http_url(candidate_url, page_url)
                if candidate and is_actionable_apply_url(candidate):
                    return candidate
            # Some providers return text with raw JS/HTML fragments.
            candidate = extract_apply_url(page_url, text)
            if candidate and is_actionable_apply_url(candidate):
                return candidate
    except Exception as exc:
        logger.warning("Bounded Exa apply-url recovery failed: %s", exc)
    return ""
...
def extract_article(item):
    ...
    try:
        safe_url=fetchable_url(url)
        if not safe_url:
            raise ValueError("Unusable HTTP URL")
        response = session.get(safe_url, headers={**HEADERS, "Referer": safe_url}, timeout=25)
        if response.status_code < 400:
            page_html = response.text
            ...
            labelled_apply = extract_labelled_apply_url(visible_text, final_url)
            if labelled_apply:
                apply_url = labelled_apply
            ...
            for key,value in labelled.items():
                ...
                if value and key in {"location","job_type","education","experience","salary","vacancies","age_limit","application_fee","application_method","application_period","selection_process"} and not item.get(key):
                    item[key]=value
            apply_url = resolve_apply_url(final_url, page_html, apply_url or labelled_apply)
            ...
            if text and len(safe_text(text)) >= 500:
                if not apply_url and safe_text(item.get("source")) in {"Bdjobs", "BDJobs Live", "Job.com.bd"}:
                    recovered_apply = exa_apply_url_fallback(final_url, safe_text(text))
                    if recovered_apply:
                        apply_url = recovered_apply
                        item["apply_url"] = recovered_apply
    ...
    try:
        result_set = get_exa().get_contents([fetchable_url(url)], text={"max_characters": 14000})
        if result_set.results:
            ...
            if text:
                ...
                for candidate_url in extract_urls_near_apply(text):
                    ...
                    if text_apply:
                        break
                text_labelled_apply = extract_labelled_apply_url(text, url)
                item["apply_url"] = resolve_apply_url(url, text, apply_url or text_labelled_apply or text_apply)
...
def build_candidate_pool(ranked, needed):
    ...
    for item in ordered:
        ...
        if len(selected)>=target:
            ...
            return selected
    # Second pass: continue in quality order but bound expensive detail work per source.
    for item in ordered:
        if len(selected)>=target: break
        can=safe_text(item.get("canonical"))
        if not can or can in seen: continue
        src=safe_text(item.get("source")) or "Unknown Source"
        if source_counts.get(src,0) >= POOL_MAX_PER_SOURCE:
            continue
...
def process_ranked_region(region, ranked):
    """Process candidates once, preserving quality/source diversity without duplicate retries."""
    pool=build_candidate_pool(ranked,MAX_STORIES_PER_RUN)
    valid=[]; attempted=0; rejected=0; skipped_source_cap=0; source_counts={}; processed=set()
    attempted_keys=set()
    ...
    def process_one(item):
        nonlocal attempted, rejected, skipped_source_cap
        key=safe_text(item.get("canonical")) or canonical_url(item.get("url", ""))
        ...
        if not canonical or canonical in processed:
            rejected += 1
            return None
        processed.add(canonical)
        story_source=safe_text(story.get("source")) or source
        if not source_allowed(story_source):
            skipped_source_cap += 1
            return None
        ...
        valid.append(story)
        source_counts[story_source]=source_counts.get(story_source,0)+1
        logger.info("ACCEPT %s #%d score=%s source=%s title=%s",region,len(valid),story.get("publish_score",0),story_source,story.get("headline",""))
        return story

    # Main pool: each candidate is attempted at most once.
    for item in pool:
        if len(valid)>=MAX_STORIES_PER_RUN:
            break
        process_one(item)

    # Rescue is strictly unattempted ranked inventory, not a retry of failed pages.
    if len(valid)<MAX_STORIES_PER_RUN:
        reserve=[]
        for item in ranked:
            if len(reserve)>=RESCUE_PROCESS_CANDIDATES:
                break
            key=safe_text(item.get("canonical")) or canonical_url(item.get("url", ""))
            if not key or key in attempted_keys:
                continue
    ...
    logger.info(
        "%s FINAL VALID: %d | target_min=%d target_max=%d | pool=%d attempted=%d rejected=%d skipped_source_cap=%d source_cap=%d sources=%s",
        region,len(valid),MIN_STORIES_PER_RUN,MAX_STORIES_PER_RUN,len(pool),attempted,rejected,skipped_source_cap,source_cap,source_counts,
    )
...
def direct_portal_gap_fill():
    ...
    for source in DIRECT_JOB_SOURCES:
        ...
        if source_is_in_cooldown(source_key, source["url"]):
            ...
            continue
        try:
            source_timeout=FRAGILE_DIRECT_SOURCE_TIMEOUTS.get(source_key,DIRECT_SOURCE_TIMEOUT_SECONDS)
            response=direct_session.get(fetchable_url(source["url"]),headers=HEADERS,timeout=source_timeout)
            ...
            if response.status_code>=400:
                ...
                continue
            soup=BeautifulSoup(response.text,"html.parser")
            resolved_listing_url=fetchable_url(response.url) or response.url
            ...
            for anchor in anchors[:max_links*4]:
                href=safe_text(anchor.get("href"))
                link=urljoin(resolved_listing_url,href)
                ...
                item=_extract_listing_anchor(source,anchor,resolved_listing_url,listing_text,source.get("new_feed",False))
                if not item:
                    continue
                seen_links.add(link)
                queue_candidate(item)
...
def run():
    global NOW_BD, TODAY_START, YESTERDAY_START, DISCOVERY_START, DISCOVERY_END, apply_exa_fallback_used
    apply_exa_fallback_used = 0
    NOW_BD = career_now()
    TODAY_START = NOW_BD.replace(hour=0, minute=0, second=0, microsecond=0)
    YESTERDAY_START = TODAY_START - timedelta(days=1)
...
def self_test():
    ...
    # Production regression guards: Unicode request targets and explicit/dynamic Apply URLs.
    unicode_url="https://www.bd-journal.com/bangladesh/312796/উচ্চশিক্ষা-ও-চাকরিতে-৫-শতাংশ-কোটা-চান-পাহাড়ি-শিক্ষার্থীরা"
    assert "%" in fetchable_url(unicode_url) and "উচ্চশিক্ষা" not in fetchable_url(unicode_url)
    apply_text="Application Method: Online Apply URL: https://apply.example.com/jobs/123"
    assert extract_labelled_apply_url(apply_text,"https://example.com/job/123")=="https://apply.example.com/jobs/123"
    assert resolve_apply_url("https://example.com/job/123", '<button onclick="window.open(\'/career/apply?id=123\')">Apply Online</button>') == "https://example.com/career/apply?id=123"
    pool_many=[{"canonical":f"pool-{i}","source":"BDJobs Live" if i<20 else "Bdjobs","importance_score":100-i,"local_score":100-i} for i in range(30)]
    pool_many=build_candidate_pool(pool_many,15)
    assert sum(1 for x in pool_many if x["source"]=="BDJobs Live") <= POOL_MAX_PER_SOURCE

    # Failed candidates are not retried in the rescue pass.
    original_processor=globals()["process_story_candidate"]
    original_basic=globals()["candidate_basic_allowed"]
    try:
        calls=[]
        globals()["candidate_basic_allowed"]=lambda item: True
        globals()["process_story_candidate"]=lambda item,verify_claims=False: calls.append(item["canonical"]) or None
        test_ranked=[{"canonical":f"selftest-{i}","source":f"S{i%3}","title":f"Job {i}","url":f"https://example.com/job/{i}","published_date":career_now().isoformat(),"excerpt":"Bangladesh job vacancy apply","importance_score":100-i,"local_score":100-i} for i in range(12)]
        assert process_ranked_region("Career",test_ranked)==[]
        assert len(calls)==len(set(calls))
    finally:
        globals()["process_story_candidate"]=original_processor
        globals()["candidate_basic_allowed"]=original_basic
