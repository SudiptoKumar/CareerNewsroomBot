...
## V1 runtime fixes applied after production log review
The first GitHub run exposed several runtime issues that the offline self-test did not cover. V1 now:
- percent-encodes Bangla/non-ASCII job URLs before requests/trafilatura use, preventing the observed `latin-1 codec` failure;
- extracts explicit `Apply URL` / `Application URL` text, relative routes, data attributes, forms and common JavaScript application routes;
- gives dynamic Bdjobs/BDJobs Live pages a bounded Exa contents fallback when local HTML has no actionable Apply URL;
- uses Exa `search()` first and keeps `search_and_contents()` only as a compatibility fallback;
- disables Cerebras SDK automatic retries and uses a local 429 cooldown so a rate limit cannot force a long hidden retry sleep;
- bounds the detailed-processing pool per source, so one portal cannot consume most of the expensive page-extraction work;
- never reprocesses a candidate that already failed extraction/apply/verification; rescue processing considers only previously unattempted inventory;
- uses a no-retry, short-timeout discovery session for fragile listing portals so one dead/blocked source cannot stall the entire run.

The tested AI SDK versions are pinned to the versions observed in the production GitHub environment: Exa `2.22.0` and Cerebras `1.91.0`.
